...
- **Token discipline.** `read_dom` returns 40 indexed elements by default with an honest cursor/remaining marker (`limit` overrides it), hides assets unless `includeAssets:true`, and returns only a small delta on unchanged repeat reads; `deltaOnly:false` forces a full read. `get_markdown` defaults to content-only extraction and a 40,000-character cap (`maxLength` overrides it); `mode:'outline'` returns headings and table shapes. Large outputs retain honest truncation markers. Full-page screenshots default to an 800px width unless overridden.
...
**Local token-diet probe (September 25, 2026):** 18 core Hermes schemas went from 45,349 to 19,907 serialized characters (−56.1%). On one IANA page in the real extension, the same three read calls returned 7,443 chars before and 5,356 after (−28.0%): `read_dom(limit:25)` 2,425→2,219, grep fallback 1,124→821, markdown 3,894→2,316. A second after run returned 1,927 / 821 / 2,316 chars respectively; the first-call hint and changing page state make the read result variable. These are payload sizes on one page, **not** a multi-task speed or model-token benchmark. Seven common tool schemas still total ~11.5k chars, above the 6k target.
